<template>
  <div class="page-container">
    ...
    <el-dialog title="确认删除" :visible.sync="deleteDialogVisible" width="300px">
      <p>确定要删除往来单位「{{ deleteItem?.name }}」吗？</p>
      <template #footer>
...
const deleteItem = ref<typeof mockContacts[0] | null>(null)
...
const total = computed(() => filteredContacts.value.length)
...
const handleAdd = () => {
  router.push('/settings/contacts/add')
}

const handleEdit = (row: typeof mockContacts[0]) => {
  router.push(`/settings/contacts/edit/${row.id}`)
}

const openDeleteConfirm = (row: typeof mockContacts[0]) => {
...
const handleDelete = () => {
  if (deleteItem.value) {
    contacts.value = contacts.value.filter(item => item.id !== deleteItem.value?.id)
  }
  deleteDialogVisible.value = false
}

const toggleStatus = (row: typeof mockContacts[0]) => {
  const index = contacts.value.findIndex(item => item.id === row.id)
  if (index !== -1) {
    contacts.value[index].status = contacts.value[index].status === 1 ? 0 : 1
  }
}
</script>
