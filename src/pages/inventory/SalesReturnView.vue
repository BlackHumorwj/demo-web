          ...
          <el-tab-pane label="基本信息" name="basic">
            <div class="view-grid">
              ...
              <div class="view-row">
                ...
                <span class="view-value">{{ doc.customerName }}</span>
              </div>
              <div class="view-row">
...
import { mockSalesReturnDocs } from '@/data/mockData'
import type { SalesReturnDoc } from '@/types/inventory'

const router = useRouter()
const route = useRoute()

const doc = ref<SalesReturnDoc | null>(null)
...
const handleEdit = () => {
  if (doc.value) {
    router.push(`/inventory/sales-return/edit/${doc.value.bizDocId}`)
  }
}

const handleDelete = () => {
  if (doc.value) {
    const index = mockSalesReturnDocs.findIndex(item => item.bizDocId === doc.value?.bizDocId)
    if (index !== -1) {
      mockSalesReturnDocs[index].status = -1
    }
    router.push('/inventory/sales-return')
  }
}
...
onMounted(() => {
  ...
  doc.value = mockSalesReturnDocs.find(item => item.bizDocId === id) || null
})
</script>
