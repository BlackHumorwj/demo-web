...
const form = reactive({
  ...
  bizProjectId: '',
  remark: '',
  categoryItems: [] as PurchaseReturnCategoryItem[],
  inventoryItems: [] as PurchaseReturnInventoryItem[]
})
...
onMounted(() => {
  const id = route.params.id as string
  const doc = mockPurchaseReturnDocs.find(item => item.bizDocId === id)

  if (doc) {
    form.bizDate = doc.bizDate
    form.supplierId = doc.supplierId
