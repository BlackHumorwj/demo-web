    ...
    <el-card class="page-card">
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px" class="form-container">
        ...
        <el-form-item label="客户" prop="ownerId">
          <el-select v-model="form.ownerId" placeholder="请选择客户" style="width: 400px" @change="onOwnerChange">
...
import type { SalesOutCategoryItem, SalesOutInventoryItem } from '@/types/inventory'
...
const createEmptyCategoryItem = (): SalesOutCategoryItem => ({
  itemId: '',
  categoryId: '',
  categoryName: '',
  amount: 0,
...
const createEmptyInventoryItem = (): SalesOutInventoryItem => ({
  itemId: '',
  prodId: '',
  prodName: '',
  prodSpec: '',
...
const form = reactive({
  ...
  bizProjectId: '',
  remark: '',
  categoryItems: [createEmptyCategoryItem()] as SalesOutCategoryItem[],
  inventoryItems: [createEmptyInventoryItem()] as SalesOutInventoryItem[]
})

const rules: FormRules = {
  bizDate: [{ required: true, message: '请选择业务日期', trigger: 'change' }],
  ownerId: [{ required: true, message: '请选择客户', trigger: 'change' }],
...
const onCategoryChange = (row: SalesOutCategoryItem) => {
  const category = mockCategories.find(c => c.id === row.categoryId)
  row.categoryName = category?.name || ''
}

const onProductChange = (row: SalesOutInventoryItem) => {
...
const updateInventoryAmount = (row: SalesOutInventoryItem) => {
  row.amount = (row.qty || 0) * (row.price || 0)
}
...
const handleBack = () => {
  router.push('/inventory/sales-out')
}
...
const handleSubmit = async () => {
  ...
  const owner = mockContacts.find(c => c.id === form.ownerId)
  ...
  const newId = String(Number(mockSalesOutDocs[mockSalesOutDocs.length - 1]?.bizDocId || '0') + 1)
  const now = new Date()
  const nowStr = now.toISOString().replace('T', ' ').substring(0, 19)
  const docNo = `SO-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(mockSalesOutDocs.length + 1).padStart(4, '0')}`

  mockSalesOutDocs.push({
    bizDocId: newId,
    bizDocNo: docNo,
    bizType: 'SALES_OUT',
