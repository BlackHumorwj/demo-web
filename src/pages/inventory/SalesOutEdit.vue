...
const form = reactive({
  ...
  bizProjectId: '',
  remark: '',
  categoryItems: [] as SalesOutCategoryItem[],
  inventoryItems: [] as SalesOutInventoryItem[]
})
...
const handleSubmit = async () => {
  ...
  const index = mockSalesOutDocs.findIndex(item => item.bizDocId === id)
...
onMounted(() => {
  const id = route.params.id as string
  const doc = mockSalesOutDocs.find(item => item.bizDocId === id)

  if (doc) {
    form.bizDate = doc.bizDate
    form.ownerType = doc.ownerType
