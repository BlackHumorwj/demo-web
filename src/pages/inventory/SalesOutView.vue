    ...
    <el-card class="page-card">
      ...
      <div v-if="doc" class="view-container">
        ...
        <el-tabs v-model="activeTab" class="detail-tabs">
          <el-tab-pane label="基本信息" name="basic">
            <div class="view-grid">
              ...
              <div class="view-row">
                ...
                <span class="view-value">{{ doc.ownerName }}</span>
              </div>
              <div class="view-row">
          ...
          <el-tab-pane label="收支明细" name="category">
...
import { mockSalesOutDocs } from '@/data/mockData'
import type { SalesOutDoc } from '@/types/inventory'

const router = useRouter()
const route = useRoute()

const doc = ref<SalesOutDoc | null>(null)
...
const handleEdit = () => {
  if (doc.value) {
    router.push(`/inventory/sales-out/edit/${doc.value.bizDocId}`)
  }
}

const handleDelete = () => {
  if (doc.value) {
    const index = mockSalesOutDocs.findIndex(item => item.bizDocId === doc.value?.bizDocId)
    if (index !== -1) {
...
onMounted(() => {
  ...
  doc.value = mockSalesOutDocs.find(item => item.bizDocId === id) || null
})
</script>
