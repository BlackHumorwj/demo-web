  ...
  <div class="page-container">
    <div class="page-header">
      <div class="search-bar">
        ...
        <el-select v-model="ownerId" placeholder="客户" class="filter-select" clearable>
      ...
      <el-button type="primary" @click="handleAdd">新增销售出库</el-button>
    </div>
    ...
    <el-card class="page-card">
      ...
      <el-table :data="pagedList" border :loading="loading">
        ...
        <el-table-column prop="ownerName" label="客户" />
...
import { mockSalesOutDocs, mockContacts } from '@/data/mockData'
import type { SalesOutDoc } from '@/types/inventory'
...
const filteredList = computed(() => {
  let list: SalesOutDoc[] = [...mockSalesOutDocs]
  ...
  if (ownerId.value) {
    list = list.filter(item => item.ownerId === ownerId.value)
  }
...
const handleAdd = () => {
  router.push('/inventory/sales-out/add')
}

const handleView = (id: string) => {
  router.push(`/inventory/sales-out/view/${id}`)
}

const handleEdit = (id: string) => {
  router.push(`/inventory/sales-out/edit/${id}`)
}

const handleDelete = (id: string) => {
  const index = mockSalesOutDocs.findIndex(item => item.bizDocId === id)
  if (index !== -1) {
    mockSalesOutDocs[index].status = -1
  }
}
</script>
