    ...
    <el-card class="page-card">
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px" class="form-container">
        ...
        <el-form-item label="供应商" prop="supplierId">
          <el-select v-model="form.supplierId" placeholder="请选择供应商" style="width: 400px" @change="onSupplierChange">
          ...
          <div class="detail-table-container">
            <el-table :data="form.categoryItems" border style="width: 100%" :key="categoryTableKey">
              ...
              <el-table-column label="金额" width="140">
                <template #default="scope">
                  <el-input v-model.number="scope.row.amount" type="number" placeholder="金额" style="width: 100%" @change="updateTotalAmount" />
                </template>
              </el-table-column>
            ...
            <div class="table-footer">
              <el-button type="primary" size="small" @click="addCategoryDetail">+ 添加支出明细</el-button>
            </div>
          </div>
        </el-form-item>
...
import type { PurchaseReturnCategoryItem, PurchaseReturnInventoryItem } from '@/types/inventory'
...
const createEmptyCategoryItem = (): PurchaseReturnCategoryItem => ({
  itemId: '',
  categoryId: '',
  categoryName: '',
  amount: 0,
...
const createEmptyInventoryItem = (): PurchaseReturnInventoryItem => ({
  itemId: '',
  prodId: '',
  prodName: '',
  prodSpec: '',
...
const form = reactive({
  ...
  bizProjectId: '',
  remark: '',
  categoryItems: [createEmptyCategoryItem()] as PurchaseReturnCategoryItem[],
  inventoryItems: [createEmptyInventoryItem()] as PurchaseReturnInventoryItem[]
})
...
const contactList = computed(() => mockContacts.filter(c => c.type === 2))
...
const onCategoryChange = (row: PurchaseReturnCategoryItem) => {
  const category = mockCategories.find(c => c.id === row.categoryId)
  row.categoryName = category?.name || ''
}

const onProductChange = (row: PurchaseReturnInventoryItem) => {
...
const updateInventoryAmount = (row: PurchaseReturnInventoryItem) => {
  row.amount = (row.qty || 0) * (row.price || 0)
}
...
const handleSubmit = async () => {
  ...
  const supplier = mockContacts.find(c => c.id === form.supplierId)
  ...
  const newId = String(Number(mockPurchaseReturnDocs[mockPurchaseReturnDocs.length - 1]?.bizDocId || '0') + 1)
  const now = new Date()
  const nowStr = now.toISOString().replace('T', ' ').substring(0, 19)
  const docNo = `PR-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(mockPurchaseReturnDocs.length + 1).padStart(4, '0')}`

  mockPurchaseReturnDocs.push({
    bizDocId: newId,
    bizDocNo: docNo,
