...
const form = reactive({
  ...
  bizProjectId: '',
  remark: '',
  categoryItems: [createEmptyCategoryItem()] as SalesReturnCategoryItem[],
  inventoryItems: [createEmptyInventoryItem()] as SalesReturnInventoryItem[]
})
...
const handleSubmit = async () => {
  ...
  const newId = String(Number(mockSalesReturnDocs[mockSalesReturnDocs.length - 1]?.bizDocId || '0') + 1)
  const now = new Date()
  const nowStr = now.toISOString().replace('T', ' ').substring(0, 19)
  const docNo = `SR-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(mockSalesReturnDocs.length + 1).padStart(4, '0')}`

  mockSalesReturnDocs.push({
    bizDocId: newId,
    bizDocNo: docNo,
