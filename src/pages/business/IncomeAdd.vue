...
const form = reactive({
  ...
  projectId: '',
  remark: '',
  details: [{ id: '', categoryId: '', categoryName: '', amount: 0, remark: '' }]
})
...
const handleSubmit = async () => {
  ...
  const hasValidDetail = form.details.some(d => d.categoryId && d.amount > 0)
  if (!hasValidDetail) {
    alert('请至少添加一条有效的明细')
    return
  }
  ...
  const newId = String(Number(mockIncomeDocs[mockIncomeDocs.length - 1]?.id || '0') + 1)
  const now = new Date()
  const docNo = `IN${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(mockIncomeDocs.length + 1).padStart(4, '0')}`

  form.details.forEach((detail, index) => {
    detail.id = `${newId}-${index + 1}`
