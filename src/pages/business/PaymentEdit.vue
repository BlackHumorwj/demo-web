...
const handleSubmit = async () => {
  ...
  const index = mockPaymentDocs.findIndex(item => item.bizDocId === form.bizDocId)
  if (index !== -1) {
    mockPaymentDocs[index] = {
      ...mockPaymentDocs[index],
...
onMounted(() => {
  ...
  const payment = mockPaymentDocs.find(item => item.bizDocId === id)
  if (payment) {
    form.bizDocId = payment.bizDocId
    form.bizDocNo = payment.bizDocNo
    form.bizDate = payment.bizDate
    form.paymentAccountId = payment.paymentAccountId
    form.accountId = payment.accountId
    form.contactId = payment.contactId
    form.paymentAmount = payment.paymentAmount
    form.currencyId = payment.currencyId
    form.exchangeRate = payment.exchangeRate
    form.bizStaffId = payment.bizStaffId
    form.bizDeptId = payment.bizDeptId
    form.bizProjectId = payment.bizProjectId
