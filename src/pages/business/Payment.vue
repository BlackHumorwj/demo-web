  ...
  <div class="page-container">
    ...
    <el-card class="page-card">
      ...
      <el-table :data="pagedList" border>
        ...
        <el-table-column prop="paymentAccountName" label="付款账户" />
        <el-table-column prop="accountName" label="挂账账户" />
        <el-table-column prop="contactName" label="往来单位" />
        <el-table-column label="付款金额" width="140">
          <template #default="scope">
            <span class="amount expense">-{{ getCurrencySymbol(scope.row.currencyId) }} {{ (scope.row.paymentAmount || 0).toLocaleString() }}</span>
          </template>
        </el-table-column>
      ...
      <div class="pagination-container">
        <span class="total-amount">总付款金额: <span class="amount expense">-{{ getCurrencySymbol('1') }} {{ totalAmount.toLocaleString() }}</span></span>
        <el-pagination
...
import { mockPaymentDocs, mockContacts, mockCurrencies } from '@/data/mockData'
...
const totalAmount = computed(() => {
  return filteredList.value.reduce((sum, item) => sum + (item.paymentAmount || 0), 0)
})
...
const handleAdd = () => {
  router.push('/business/payment/add')
}

const handleView = (row: PaymentDoc) => {
  router.push(`/business/payment/view/${row.bizDocId}`)
}

const handleEdit = (row: PaymentDoc) => {
  router.push(`/business/payment/edit/${row.bizDocId}`)
}

const handleDelete = (row: PaymentDoc) => {
  if (confirm(`确定要删除付款单 ${row.bizDocNo} 吗？`)) {
    const idx = mockPaymentDocs.findIndex(item => item.bizDocId === row.bizDocId)
    if (idx !== -1) {
      mockPaymentDocs[idx].status = -1
    }
  }
}
</script>
