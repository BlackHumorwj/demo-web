    ...
    <el-card class="page-card">
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px" class="form-container">
        ...
        <el-form-item label="往来单位" :required="isArapAccount">
          <el-select v-model="form.bizContactId" placeholder="请选择往来单位" style="width: 400px" clearable>
...
const form = reactive({
  ...
  exchangeRate: 1.0,
  remark: '',
  categoryItems: [] as ExpenseCategoryItem[]
})
...
const handleSubmit = async () => {
  ...
  form.categoryItems.forEach((detail, index) => {
    if (!detail.itemId) {
      detail.itemId = `${form.bizDocId}-${index + 1}`
    }
  ...
  const index = mockExpenseDocs.findIndex(item => item.bizDocId === form.bizDocId)
  if (index !== -1) {
    mockExpenseDocs[index] = {
      ...mockExpenseDocs[index],
...
onMounted(() => {
  ...
  const expense = mockExpenseDocs.find(item => item.bizDocId === id)
  if (expense) {
    form.bizDocId = expense.bizDocId
    form.bizDocNo = expense.bizDocNo
    form.tradeTime = expense.tradeTime
    form.accountId = expense.accountId
    form.staffId = expense.staffId
    form.deptId = expense.deptId
    form.projectId = expense.projectId
    form.bizContactId = expense.bizContactId
    form.amount = expense.amount
    form.currencyCode = expense.currencyCode
    form.exchangeRate = expense.exchangeRate
    form.remark = expense.remark
    form.categoryItems = (expense.categoryItems || []).map(d => ({ ...d }))
    if (form.categoryItems.length === 0) {
      form.categoryItems.push({ itemId: '', categoryId: '', categoryName: '', amount: 0, remark: '' })
    }
  } else {
    router.push('/business/expense')
