...
const handleSubmit = async () => {
  ...
  const now = new Date()
  const newId = 'R' + String(Date.now())
  const docNo = `RC${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(mockReceiptDocs.length + 1).padStart(4, '0')}`

  const newDoc: ReceiptDoc = {
    bizDocId: newId,
    bizDocNo: docNo,
    bizType: '收款',
