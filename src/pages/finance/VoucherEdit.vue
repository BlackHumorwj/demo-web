<template>
  <div class="page-container">
    ...
    <el-card v-if="loadedOnce" class="entry-card">
      <template #header>
    ...
    <div class="action-bar" v-if="loadedOnce">
...
const fetchDetail = () => {
  ...
  const voucher = mockVouchers.find(v => parseInt(v.id) === id)
  if (!voucher) {
    ElMessage.error('凭证不存在')
    router.push('/finance/voucher')
    return
  }

  form.voucherCode = voucher.voucherNo
  form.voucherDate = voucher.date
  form.remark = voucher.abstract || ''
  form.bizDocType = voucher.type === 'debit' ? 1 : 2
  form.bizDocNo = `IN-202406-${String(id).padStart(3, '0')}`
  form.attachmentCount = 0
  form.attachmentIds = ''

  const firstCode = voucher.type === 'debit' ? '1002' : '6602'
  const secondCode = voucher.type === 'debit' ? '6001' : '1002'
  const firstName =
    mockSubjects.find(s => s.code === firstCode)?.name ||
    (voucher.type === 'debit' ? '银行存款' : '管理费用')
  const secondName =
    mockSubjects.find(s => s.code === secondCode)?.name ||
    (voucher.type === 'debit' ? '主营业务收入' : '银行存款')

  form.entryList = [
    {
      ...createEmptyEntry(),
      summary: voucher.abstract,
      subjectCode: firstCode,
      subjectName: firstName,
      debitAmount: voucher.type === 'debit' ? voucher.debitAmount : null,
      creditAmount: voucher.type === 'credit' ? voucher.creditAmount : null
    },
    {
      ...createEmptyEntry(),
      summary: voucher.abstract,
      subjectCode: secondCode,
      subjectName: secondName,
      debitAmount: voucher.type === 'credit' ? voucher.debitAmount : null,
      creditAmount: voucher.type === 'debit' ? voucher.creditAmount : null
    }
  ]
...
const handleSubmit = async () => {
  ...
  const voucher = mockVouchers.find(v => parseInt(v.id) === id)
  if (voucher) {
    voucher.date = form.voucherDate
    voucher.abstract = form.remark || form.entryList[0]?.summary || voucher.abstract
    voucher.debitAmount = debitTotal.value
    voucher.creditAmount = creditTotal.value
  }

  ElMessage.success('凭证修改成功')
  router.push('/finance/voucher')
}
