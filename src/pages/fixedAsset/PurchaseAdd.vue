...
import { mockFixedAssetPurchases, mockFixedAssets, mockAssetCategories, mockDepartments, mockStaff, mockAccounts, mockContacts, FixedAssetPurchase } from '@/data/mockData'
...
const handleSubmit = () => {
  formRef.value.validate((valid: boolean) => {
    ...
    const newPurchase: FixedAssetPurchase = {
      bizDocId: String(mockFixedAssetPurchases.length + 1),
      bizDocNo: `CG-${dateStr.replace(/-/g, '')}-${String(mockFixedAssetPurchases.length + 1).padStart(4, '0')}`,
    ...
    validAssets.forEach((assetItem) => {
      ...
      newPurchase.assetList.push(newAsset)
    })
    
    mockFixedAssetPurchases.push(newPurchase)
    alert('保存成功')
    router.push('/fixedAsset/purchase')
  })
}
