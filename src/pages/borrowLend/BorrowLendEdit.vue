...
const formRef = ref()
const originalArapType = ref<number | null>(null)
...
const handleSubmit = () => {
  ...
  const index = mockBorrowLendList.findIndex(item => item.bizDocId === form.value.bizDocId)
  if (index !== -1) {
    mockBorrowLendList[index] = {
      ...mockBorrowLendList[index],
...
onMounted(() => {
  ...
  const record = mockBorrowLendList.find(item => item.bizDocId === id)
  if (record) {
    form.value = {
      bizDocId: record.bizDocId,
      tradeTime: record.tradeTime,
      arapType: record.arapType,
      accountId: record.accountId,
      fundAccountId: record.fundAccountId,
      bizContactId: record.bizContactId,
    ...
    originalArapType.value = record.arapType
  } else {
    alert('记录不存在')
    router.push('/borrowLend')
  }
