      ...
      <el-table :data="filteredList" border>
        ...
        <el-table-column prop="arapTypeName" label="挂账类型" width="100">
          <template #default="scope">
            <el-tag
              :type="scope.row.arapType === 5 ? 'success' : 'warning'"
            >
...
import { mockBorrowLendList, mockContacts } from '@/data/mockData'
...
const handleAdd = () => {
  router.push('/borrowLend/add')
}

const handleView = (bizDocId: string) => {
  router.push(`/borrowLend/view/${bizDocId}`)
}

const handleEdit = (bizDocId: string) => {
  router.push(`/borrowLend/edit/${bizDocId}`)
}

const handleDelete = (bizDocId: string) => {
  if (confirm('确定要删除这条借入借出记录吗？')) {
    const index = mockBorrowLendList.findIndex(item => item.bizDocId === bizDocId)
    if (index !== -1) {
      mockBorrowLendList[index].status = -1
      mockBorrowLendList[index].statusName = '已删除'
      alert('删除成功')
    }
  }
}
