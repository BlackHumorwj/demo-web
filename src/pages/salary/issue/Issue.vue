<template>
  <div class="page-container">
    <div class="page-header">
      ...
      <el-button type="primary" @click="handleAdd">新增工资发放</el-button>
    </div>
    ...
    <el-card class="page-card">
      ...
      <el-table :data="filteredIssueList" border :loading="loading">
        <el-table-column prop="bizDocNo" label="单据编号" />
        <el-table-column prop="salaryPeriod" label="工资期间" />
        <el-table-column prop="accruedNo" label="关联计提单" />
        <el-table-column prop="accountName" label="发放账户" />
        <el-table-column prop="tradeTime" label="发放日期" />
        ...
        <el-table-column label="操作" width="200">
          <template #default="scope">
            ...
            <el-button v-if="scope.row.docStatus === 0" type="text" size="small" style="color: #3b82f6" @click="handleConfirm(scope.row.bizDocId)">发放</el-button>
      ...
      <div class="pagination-container">
        <span class="total-count">共 {{ filteredIssueList.length }} 条记录</span>
        <el-pagination
          v-model:current-page="currentPage"
          v-model:page-size="pageSize"
          :total="filteredIssueList.length"
...
const filteredIssueList = computed(() => {
  let list = [...mockSalaryIssueList]
  
  if (searchKeyword.value) {
    const keyword = searchKeyword.value.toLowerCase()
    list = list.filter(item => 
      item.bizDocNo.toLowerCase().includes(keyword) ||
      item.accruedNo.toLowerCase().includes(keyword) ||
...
const handleAdd = () => {
  router.push('/salary/issue/add')
}

const handleView = (id: number) => {
  router.push(`/salary/issue/view/${id}`)
}

const handleEdit = (id: number) => {
  router.push(`/salary/issue/edit/${id}`)
}

const handleDelete = (id: number) => {
  const index = mockSalaryIssueList.findIndex(item => item.bizDocId === id)
  if (index !== -1) {
    mockSalaryIssueList.splice(index, 1)
  }
}

const handleConfirm = (id: number) => {
  const item = mockSalaryIssueList.find(item => item.bizDocId === id)
  if (item) {
    item.docStatus = 1
    item.docStatusName = '已发放'
  }
}
