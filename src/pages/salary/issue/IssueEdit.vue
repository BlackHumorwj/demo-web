...
import { mockSalaryAccruedList, mockSalaryIssueList, mockAccounts } from '@/data/mockData'
import type { SalaryIssue, SalaryIssueDetail } from '@/data/mockData'
...
const handleSave = () => {
  ...
  if (issueData.value) {
    const account = mockAccounts.find(a => parseInt(a.id) === formData.value.accountId)
    
    issueData.value.salaryPeriod = formData.value.salaryPeriod
    issueData.value.tradeTime = formData.value.tradeTime
    issueData.value.accruedId = formData.value.accruedId
    issueData.value.accountId = formData.value.accountId
    issueData.value.accountName = account?.name || ''
    issueData.value.remark = formData.value.remark
    issueData.value.items = formData.value.items
    issueData.value.amount = totalAmount.value
  }
...
onMounted(() => {
  ...
  if (issueData.value) {
    formData.value = {
      salaryPeriod: issueData.value.salaryPeriod,
      tradeTime: issueData.value.tradeTime,
      accruedId: issueData.value.accruedId,
      accountId: issueData.value.accountId,
      remark: issueData.value.remark,
      items: JSON.parse(JSON.stringify(issueData.value.items))
    }
  }
})
</script>
