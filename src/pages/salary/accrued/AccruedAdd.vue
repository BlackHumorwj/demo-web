...
import type { SalaryAccruedDetail } from '@/data/mockData';
...
const handleSave = () => {
 ...
 const newAccrued = {
 bizDocId: Date.now(),
 bizDocNo: `SAC-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${String(mockSalaryAccruedList.length + 1).padStart(4, '0')}`,
 bizDocType: 270,
 salaryPeriod: formData.value.salaryPeriod,
 tradeTime: formData.value.tradeTime,
 grossAmount: totalGrossAmount.value,
 taxAmount: totalTaxAmount.value,
 otherAmount: totalOtherAmount.value,
 ...
 mockSalaryAccruedList.push(newAccrued);
 alert('保存成功');
