...
const handleSubmit = () => {
  ...
  const newRecord = {
    bizDocId: String(mockAdvanceList.length + 1),
    bizDocNo: `AD-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${String(mockAdvanceList.length + 1).padStart(4, '0')}`,
  ...
  mockAdvanceList.push(newRecord)
  alert('保存成功')
  router.push('/advance')
}
</script>
