          ...
          <div class="detail-table-container">
            <el-table :data="form.items" border style="width: 100%" :key="tableKey">
              ...
              <el-table-column label="数量" width="120">
                <template #default="scope">
                  <el-input v-model.number="scope.row.quantity" type="number" placeholder="数量" style="width: 100%" @change="updateAmount(scope.row)" />
                </template>
              </el-table-column>
              <el-table-column label="单价" width="120">
                <template #default="scope">
                  <el-input v-model.number="scope.row.price" type="number" placeholder="单价" style="width: 100%" @change="updateAmount(scope.row)" />
                </template>
              </el-table-column>
...
const form = reactive({
  ...
  remark: '',
  items: [{ productId: '', productName: '', spec: '', quantity: 0, price: 0, remark: '' }] as PurchaseItem[]
})
...
const updateAmount = (row: PurchaseItem) => {
}
...
const handleSubmit = async () => {
  ...
  const newId = String(Number(mockPurchaseInDocs[mockPurchaseInDocs.length - 1]?.bizDocId || '0') + 1)
  const now = new Date()
  const nowStr = now.toISOString().replace('T', ' ').substring(0, 19)
  const docNo = `PI-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(mockPurchaseInDocs.length + 1).padStart(4, '0')}`

  let itemIdCounter = 1
  form.items.forEach((detail, index) => {
    detail.productName = mockProducts.find(p => p.id === detail.productId)?.name || ''
    const itemId = `${newId}-${itemIdCounter++}`
    form.items[index].productName = detail.productName
  })

  mockPurchaseInDocs.push({
    bizDocId: newId,
    bizDocNo: docNo,
